...
class sinIR_direct(CellModel):
    def __init__(self,*args,**kwargs):
        ...
        self.spo_threshold=2.5


def f(mode):
    n=1000
    resume=False
    if mode in ["n100","n10","n1"]:
        m=sinIR_direct(minimal_time_interval=0.1)
        m.sim_cell_lines(setting="wt",mode="mode",minimal_time_interval=0.1,max_time=16,num_of_lines=n,resume=resume,save_name='test_spo_gr_change_bgdeg_'+mode)
        m.generate_avg_figures(['TapA'],save_name='test_spo_gr_change_bgdeg_'+mode,xlim=(2,20),ylim=(0,100),plot_traj=False)
    elif mode in ["da","dc","ds"]:
        m=sinIR_direct(minimal_time_interval=0.1)
        m.sim_cell_lines(setting=mode,mode="n100",minimal_time_interval=0.1,max_time=16,num_of_lines=n,resume=resume,save_name='test_spo_full_bgdeg_'+mode)
        m.generate_avg_figures(['TapA'],save_name='test_spo_full_bgdeg_'+mode,xlim=(2,20),ylim=(0,100),plot_traj=False)
