...
def f(mode):
    if mode=="ds_1":
        ...
        resume=False
        m.sim_cell_lines(setting="ds",mode="feedback",minimal_time_interval=0.1,max_time=16,num_of_lines=1000,resume=resume,save_name='ds_feedback_bgdeg')
        m.generate_avg_figures(['TapA'],save_name='ds_feedback_bgdeg',xlim=(2,20),ylim=(0,100),plot_traj=False)
    elif mode=="ds_2":
        m=sinIR_direct(minimal_time_interval=0.1)
        resume=True
        m.sim_cell_lines(setting="ds",mode="n100",minimal_time_interval=0.1,max_time=16,num_of_lines=100,resume=resume,save_name='ds_direct')
        m.generate_avg_figures(['TapA'],save_name='ds_direct',xlim=(2,20),ylim=(0,100),plot_traj=False)
    elif mode in ["n100","n10","n1"]:
        m=sinIR_full(minimal_time_interval=0.1)
        resume=False
        m.sim_cell_lines(setting="wt",mode="mode",minimal_time_interval=0.1,max_time=16,num_of_lines=100,resume=resume,save_name='gr_change_bgdeg_'+mode)
        m.generate_avg_figures(['TapA'],save_name='gr_change_bgdeg_'+mode,xlim=(2,20),ylim=(0,100),plot_traj=False)
    elif mode in ["da","dc","ds"]:
        m=sinIR_full(minimal_time_interval=0.1)
        resume=False
        m.sim_cell_lines(setting=mode,mode="n100",minimal_time_interval=0.1,max_time=16,num_of_lines=100,resume=resume,save_name='full_bgdeg_'+mode)
